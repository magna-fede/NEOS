...
from my_eyeCA import preprocess, ica_andapply
...
bad_eeg = config.bad_channels[sbj_id]['eeg']
	...
	# %%
for i, drf in enumerate(data_raw_files):
    ...
    ica_file = path.join(sbj_path, 'ICA',
                             f'block{i+1}_sss_raw_ICA_extinfomax_0.99_COV_None',
                             f'block{i+1}_sss_raw_ICA_extinfomax_0.99_COV_None-ica')

	# %%
    raw, ic, ic_scores = ica_andapply.apply_ica_pipeline(raw=raw, evt=evt, thresh=variance_threshold,
    						ica_filename=ica_file)
