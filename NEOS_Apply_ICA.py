...
def run_Apply_ICA(sbj_id):
    ...
    for raw_stem_in in sss_map_fnames:
        ...
        FileICA = path.join(sbj_path, raw_stem_in[:-7] + 'sss_f_raw')

        FileRawIn = path.join(sbj_path, raw_stem_in[:-7] + 'sss_f_raw')
        ...
        FileRawOut = path.join(sbj_path, raw_stem_in[:-7] + 'sss_f_' +
                             config.raw_ICA_suff)
