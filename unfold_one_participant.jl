...
c = h5open("/imaging/hauk/users/fm02/MEG_NEOS/jl_evts/data_sbj_$sbj_id.h5", "r") do file
    read(file, "dataset_1")
end
...
results = coeftable(m)
plot_erp(results)
CSV.write("/imaging/hauk/users/fm02/MEG_NEOS/jl_evts/coeftable_$(sbj_id)_concpred.csv", results, transform=(col, val) -> something(val, missing))

eff = effects(Dict(:Predictability => ["Predictable",
                                       "Unpredictable"]),m)

CSV.write("/imaging/hauk/users/fm02/MEG_NEOS/jl_evts/effect_$(sbj_id)_pred.csv", eff, transform=(col, val) -> something(val, missing))

eff = effects(Dict(:Concreteness => ["Concrete",
                                       "Abstract"]),m)

CSV.write("/imaging/hauk/users/fm02/MEG_NEOS/jl_evts/effect_$(sbj_id)_conc.csv", eff, transform=(col, val) -> something(val, missing))
