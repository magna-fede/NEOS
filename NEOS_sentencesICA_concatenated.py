...
def run_ICA(sbj_id):
    ...
    epochs_concatenated = np.concatenate(data_selection['data'], axis=1)
    
    # epochs must be same length
    epochs_for_ica = mne.io.BaseRaw(info=data.info, preload=epochs_concatenated)
    ...
    print(f'Saving ICA to {path.join(sbj_path, sbj_path[-3:] + "_sss_f_raw-ica_concatenated.fif")}')
    ica.save(path.join(sbj_path, sbj_path[-3:] + '_sss_f_raw-ica_concatenated.fif'), overwrite=True)
    report.save(path.join(sbj_path, 'Figures', 'report_ica_concatenated.html'), overwrite=True)
