...
for block, drf in enumerate(data_raw_files):
    ...
    ic = ica.run_ica_pipeline(
	    raw=raw, evt=evt, method="extinfomax", cov_estimator=None, n_comp=0.99, drf=drf
	)
    ...
    sns.despine(fig)
    fname_fig = path.join(sbj_path, 'Figures', f'snr_metrics_ICA_componentselection_{sbj_id}_{block}_NOoverweight.png')
