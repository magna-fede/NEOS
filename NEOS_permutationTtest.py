...
#conditions = ['predictable', 'unpredictable'] 
...
for condition in conditions:
    ...
    for sbj_id in sbj_ids:
        ...
        stc_fname = path.join(stc_path, 
                              f"{subject}_stc_{condition}_normalorientation_fsaverage")
...
# cond1 = np.stack([stcs['predictable'][i].data for i in range(len(stcs['predictable']))])
# cond2 = np.stack([stcs['unpredictable'][i].data for i in range(len(stcs['unpredictable']))])
...
# with open('/imaging/hauk/users/fm02/MEG_NEOS/data/misc/cluster1sampPredictability_normalorientation.P', 'wb') as f:
#     pickle.dump(clu, f)

with open('/imaging/hauk/users/fm02/MEG_NEOS/data/misc/cluster1sampConcreteness_normalorientation.P', 'wb') as f:
    pickle.dump(clu, f)
