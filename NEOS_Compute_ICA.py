...
class create_args:
  def __init__(self, FileRaw, FileICA, FileHTML):
    ...
    self.EOG = ['EOG001','EOG002']
    self.ECG = ['synth']  # test 'synth' option if no ECG channel present
    self.maxEOG = 2
    self.maxECG = 2
...
def run_Compute_ICA(sbj_id):
    ...
    for raw_stem_in in sss_map_fnames:
        ...
        FileRaw = path.join(sbj_path, raw_stem_in[:-7] + 'sss_f_raw')
    ...
    FileICA = path.join(sbj_path, config.map_subjects[sbj_id][0] + '_sss_f_raw')

    # -ica.html will be appended
    FileHTML = path.join(sbj_path, config.map_subjects[sbj_id][0] + '_sss_f_raw')
