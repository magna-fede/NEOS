...
for block, drf in enumerate(data_raw_files):
    ...
    sns.despine(fig)
    fname_fig = path.join(sbj_path, 'Figures', f'snr_metrics_ICA_filtering_{sbj_id}_{block+1}_overweight_both.png')
    fig.savefig(fname_fig) 
    
    df.to_csv(path.join(sbj_path, f"snr_compare_filt_{sbj_id}_{block+1}_overweight_both.csv"))
