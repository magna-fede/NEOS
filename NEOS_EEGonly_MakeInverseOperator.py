...
def make_InverseOperator(sbj_id):
    ...
    fwd_eeg = mne.read_forward_solution(fwd_fname)

    fname_cov = path.join(sbj_path, config.map_subjects[sbj_id][0][-3:] + \
                              'EEGonly_covariancematrix_auto-cov.fif')
    ...
    invop_eeg = mne.minimum_norm.make_inverse_operator(info, fwd_eeg, noise_cov,
                                                          fixed='auto', loose=loose, depth=depth,
                                                          rank='info')

    inv_fname = path.join(sbj_path, subject + '_EEG-inv.fif')
    print('Writing EEG/MEG inverse operator: %s.' % inv_fname)
    mne.minimum_norm.write_inverse_operator(fname=inv_fname, inv=invop_eeg)
